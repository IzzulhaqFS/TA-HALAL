...
@section('subhead')
    <title>Pengecekan Bahan Jerohan</title>
@endsection
...
@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 id="main-header" class="text-lg font-medium mr-auto main-activity" 
            data-pos="0" 
            data-label="Cek Jerohan" 
            data-value="Syubhat">
            Pengecekan Bahan Jerohan
        </h2>
    </div>
    @if ($errors->any())
            ...
            <div class="intro-y box">
                ...
                <div id="input" class="p-5">
                    <div class="preview">
                        ...
                        {{-- BEGIN: Form --}}
                        <form id="cek-jerohan-form" action="{{ route('hewani.bahan-baku.process', ['ingredient_id' => $ingredient->id]) }}" method="GET">
                            <div class="mt-3">
                                ...
                                <label for="regular-form-1" class="form-label">Apakah hewan asal jerohan diketahui? <span class="text-danger">*</span></label>
                                <select id="asal-hewan-diketahui-select" class="form-control" name="asal-hewan-diketahui">
                                    <option value="">-- Pilih --</option>
                                    <option value="1" {{ old('asal-hewan-diketahui') == '1' ? 'selected' : '' }} class="sub-activity" data-pos="0" data-label="Apakah hewan asal jerohan diketahui?">Diketahui</option>
                                    <option value="0" {{ old('asal-hewan-diketahui') == '0' ? 'selected' : '' }} class="sub-activity" data-pos="0" data-label="Apakah hewan asal jerohan diketahui?">Tidak Diketahui</option>
                                </select>
                            </div>
    
                            {{-- BEGIN: Cek Asal Hewan Jerohan--}}
                            <div id="asal-hewan-halal" class="asal-hewan-halal main-activity" 
                                data-pos="1"
                                data-label="Cek Asal Hewan Bahan Jerohan"
                                data-value="">
                                <div class="mt-4">
                                    <label for="regular-form-1" class="form-label">Apakah jerohan berasal dari hewan halal? <span class="text-danger">*</span></label>
                                    <select id="asal-hewan-select" class="form-control" name="asal-hewan">
                                        <option value="">-- Pilih --</option>
                                        <option value="1" {{ old('asal-hewan') == '1' ? 'selected' : '' }} class="sub-activity" data-pos="1" data-label="Apakah jerohan berasal dari hewan halal?">Iya</option>
                                        <option value="0" {{ old('asal-hewan') == '0' ? 'selected' : '' }} class="sub-activity" data-pos="1" data-label="Apakah jerohan berasal dari hewan halal?">Tidak</option>
                                    </select>
                                </div>
                                <div class="mt-3">
                            ...
                            {{-- END: Cek Asal Hewan Jerohan --}}
    
                            {{-- BEGIN: Cek COA Babi Jerohan --}}
                            <div id="uji-babi-detail" class="uji-babi-detail main-activity" 
                                data-pos="2"
                                data-label="Cek COA DNA Babi pada Jerohan"
                                data-value="">
                                <div class="mt-4">
    ...
    <script>
        document.getElementById('right-btn').addEventListener('click', function(e) {
            ...
            let form = document.querySelector('#cek-jerohan-form');
            form.submit(); 
        })
    </script>
